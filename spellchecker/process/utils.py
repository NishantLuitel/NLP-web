...
with open('NLP_Trained_models/spellchecker/data/trie_depth.pickle', 'rb') as f:
    trie_depth = pickle.load(f)

with open('NLP_Trained_models/spellchecker/data/depth_dict.pickle', 'rb') as f:
...
def check_distance2(w, depth=1, edit_distance=2):
    ...
    words = depth_dict[depth]
    candidates = list(
        filter(lambda word: textdistance.levenshtein.distance(w, word) <= edit_distance, words))
...
def candidate_words_trie(word, minimum=1, start_depth=0, edit_probabs=None):
    ...
    if len(word) <= 3:
        ...
        for i in range(len(depth_dict)-1):
            if c_ < minimum:
                c, c_ = check_distance_trie(word, depth=start_depth+i+1,
                                            edit_distance=ed1, candidates=c)
    ...
    # If word length is more than 3 than use edit distance of 2 or less
        ...
        for i in range(len(trie_depth)-1):
            if c_ < minimum:
                #print("Entered depth, ",i+1)
                c, c_ = check_distance_trie(word, depth=start_depth+i+1,
                                            edit_distance=ed2, candidates=c)
...
def final_candidate_words(word, minimum=1, top=5, start_depth=0, force=False, use_trie=False, time=False):
    ...
    c = candidate_words(word, minimum=minimum, start_depth=start_depth) if use_trie == False else candidate_words_trie(
        word, minimum=minimum, start_depth=start_depth)
