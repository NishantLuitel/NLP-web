...
class KneserNeyLM:
    ...
    def generate_sentence(self, sent, min_length=10):
        """
        ...
        while len(sent) < min_length + self.highest_order:
            sent = [self.start_pad_symbol] * (self.highest_order - 1) + sent
            # Append first to avoid case where start & end symbal are same
            sent.append(self.generate_next_word(sent))
    ...
    def _get_context_order(self, sentence, order):
        """
        Extract context to predict next word from sentence.
    ...
    def perplexity(self, sent):
        ...
        sent = [self.start_pad_symbol] * (self.highest_order - 1) + sent
        logprob = self.logprob(tuple(sent))
        per = math.pow(math.exp(logprob), -1/sentence_len)
        return per

    def generate_next_word(self, sent):
        # probs=kn_lm3.lm[0]
        for i in range(self.highest_order):
            context = tuple(self._get_context_order(sent, self.highest_order-i))
            pos_ngrams = [
                (ngram, logprob) for ngram, logprob in self.lm[i].items()
